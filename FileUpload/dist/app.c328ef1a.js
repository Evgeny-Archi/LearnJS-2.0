...
function upload(selector) {
  ...
  var files = []; // Глобальная переменная для доступа к добавленным картинкам
  // Создаем псевдо-кнопку

  var openButton = createElem('button', ['btn'], 'Открыть');
  input.insertAdjacentElement('afterend', openButton);
  var uploadButton = createElem('button', ['btn', 'primary'], 'Загрузить');
  openButton.insertAdjacentElement('afterend', uploadButton);
  uploadButton.style.display = 'none'; // Настройка инпута
  ...
  var changeHandler = function changeHandler(event) {
    ...
    files.forEach(function (file) {
      ...
      reader.readAsDataURL(file); // Читаем файлы и добавляем на страницу превью

      reader.onload = function (ev) {
        var src = ev.target.result; // Получаем картинку в формате base64

        preview.insertAdjacentHTML('afterbegin', "\n                <div class=\"preview-image\">\n                    <div class=\"preview-remove\" data-name=\"".concat(file.name, "\">&times;</div>\n                    <img src=\"").concat(src, "\" alt=\"").concat(file.name, "\">\n                    <div class=\"preview-info\">\n                        <span class=\"preview-info__name\">").concat(cutText(file.name), "</span>\n                        <span class=\"preview-info__size\">").concat(bytesToSize(file.size), "</span>\n                    </div>\n                    <div class=\"output-url\"></div>\n                </div>\n                "));
      };
    });
  };
  ...
  var removeHandler = function removeHandler(event) {
    ...
    var name = event.target.dataset.name; // Получаем имя картинки из дата атрибута кнопки
  ...
  var uploadHandler = function uploadHandler() {
    preview.querySelectorAll('.preview-remove').forEach(function (item) {
      return item.remove();
    }); // Убираем кнопки удаления во время загрузки

    var outputUrl = preview.querySelectorAll('.output-url');
    var previewInfo = preview.querySelectorAll('.preview-info');
    previewInfo.forEach(function (item) {
      return clearBlock(item);
    }); // Меняем строку с информацией на полосу загрузки
...
(0, _upload.upload)('#file', {
  ...
  onUpload: function onUpload(files, blocks, outputUrl) {
    files.forEach(function (file, index) {
      // Передаем картинку firebase
      var ref = storage.ref("images/".concat(file.name));
      var task = ref.put(file); // Отслеживаем состояние загрузки
...
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  ...
  var ws = new WebSocket(protocol + '://' + hostname + ':' + "38903" + '/');
